...
EOD;
	...
	public function addPostOrPage($req){
		if(strlen($req) > 0){
			$id = $title = $cat = $text = $filter = $slug = $url = $publDate = $createDate = $type = null;
			$blogPre = "blogpost-";
			...
			if(isset($txtFilter)){
				...
				// put default filter if no filter
				if (strlen($filter) <= 0){
					$filter = "nl2br";
				}
			...
			$sqlInsert = "INSERT INTO " . strip_tags($this->tableName) . " (" . "id, category, slug, url, TYPE, title, DATA, FILTER, published, created) ";
			
			$sqlInsert .= "VALUES ( " . strip_tags($id) . ",'" . strip_tags($cat) . "','" . strip_tags($slug);
	...
	public function trash($query){
		...
		if(isset($delete) AND $delete == "true"){
			...
			if($this->dBase->RowCount() == 1){ // RowCount() return int number of affected rows of last statement
				return "<br><H2>Blogpost raderad</H2>";
			}else{
				return "<br><H2>Blogpost INTE raderad</H2>" . mysql_error();
			}
			...
			if($this->dBase->RowCount() == 1){ // RowCount() return int number of affected rows of last statement
				return "<br><H2>Blogpost slängd</H2>";
			}else{
				return "<br><H2>Blogpost INTE slängd</H2>" . mysql_error();
			}
		}else{
