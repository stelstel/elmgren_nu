  ...
  */
  public function __construct($db, $URI, $dbTab) {
	...
	private function handleHTTPRequest($req){
		...
		$this->handledReqVar["pag"] = $this->handledReqVar["sortOrder"] = $this->handledReqVar["ascendOrDescend"] = $this->handledReqVar["cat"] = null;
		$defaultLimit = 4;
		...
		if(isset($arrInputReq["category"])){
			$this->handledReqVar["cat"] = $arrInputReq["category"];
		}
...
EOD;
		...
		foreach($result AS $key=>$value){
  		...
  		$output .= '<td style="text-align: center">';
			...
			if($this->inlogging->isAuth() ){
				...
				$output .= '<td><a href="trashMov.php?trash=true&id=' . $value->id . '">';
	...
	private function makeSQLQuery($reqVar){
		...
		if($anyValsInReqVar){
			...
			if(isset($reqVar["cat"]) AND $reqVar["cat"] > 0){
				$sqlCat = "SELECT movie2genre.idMovie FROM movie2genre WHERE movie2genre.idGenre=" . $reqVar["cat"];
				$result = $this->dataBase->ExecuteSelectQueryAndFetchAll($sqlCat);
				$this->sqlQuery .= " AND ";
				$j = 0;
				
				foreach($result AS $key=>$value){
					$this->sqlQuery .= "id=" . $value->idMovie . " ";
					if(count($result) > $j+1){
						$this->sqlQuery .= " OR ";			
					}
					$j++;
	...
	//************************************************************************************
	public function getCategories($filmId){
		$sql = "SELECT name, id FROM genre INNER JOIN movie2genre ON movie2genre.idGenre = genre.id WHERE movie2genre.idMovie =" . $filmId;
		...
		$tempCat = $this->handledReqVar["cat"]; //To keep original URL category
		
		foreach($result AS $key=>$value){
			$this->handledReqVar["cat"] = $value->id;
			$this->handledReqVar["pag"] = 1;
			$output .= '<a href="' . $this->addKeyValsToOutURL() . '">';
		...
		$this->handledReqVar["cat"] = $tempCat; // Back to original URL category
		return $output;
	}
	...
	private function addKeyValsToOutURL(){
		...
		if(isset($this->handledReqVar["cat"]) AND strlen($this->handledReqVar["cat"]) > 0 ){
			$output .= '&category=' . $this->handledReqVar["cat"];	
		}
