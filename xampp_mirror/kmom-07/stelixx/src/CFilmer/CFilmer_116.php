...
EOD;
	...
	private function getCategories($filmId){
		...
		$i = 0;
		$this->handledReqVar["category"] = "";
		
		foreach($result AS $key=>$value){
				$this->handledReqVar["category"] .= '<a href="' . 'filmer.php?category=' . $value->id . '">';
				$this->handledReqVar["category"] .= ucfirst($value->name);
				$this->handledReqVar["category"] .= '</a>';
				if($i < $arrSize - 1 ){
					$this->handledReqVar["category"] .= ", ";	
				}
				$i++;
		}
		return $pageURL . $this->addKeyValsToOutURL();
	}
