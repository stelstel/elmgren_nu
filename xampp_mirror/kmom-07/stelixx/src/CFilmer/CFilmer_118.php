...
EOD;
	...
	private function getCategories($filmId){
		...
		foreach($result AS $key=>$value){
			$this->handledReqVar["category"] = $value->id;
	...
	private function addKeyValsToOutURL(){
		...
		if(isset($this->handledReqVar["category"]) AND strlen($this->handledReqVar["category"]) > 0 ){
			$output .= '&category=' . $this->handledReqVar["category"];	
		}
