...
EOD;
	...
	public function createResultTable() {
		...
		foreach($result AS $key=>$value){
  		...
  		$output .= '<td style="text-align: center">';
			...
			if($this->inlogging->isAuth() ){
				$output .= '<td><a href="editmovie.php?id=' . $value->id . '">Redigera</td>';
			}
	...
	//*****************************************************************************
	public function createSimpleResultTable() {
		$output = "";
				
		$output .= "<table class='film_tbl' cellpadding='3' width='950' align='center' border='1' ><tr>";
		$output .= "<th scope='col'>Titel </th><th scope='col'></th>";
		$sqlSimple = "SELECT id, title FROM movie ORDER BY title";
						
		$result = $this->dataBase->ExecuteSelectQueryAndFetchAll($sqlSimple);
		
		foreach($result AS $key=>$value){
			//$movieLink = '<a href="film.php?id=' . $value->id . '">';
			$output .=	"<tr>";
  		$output .= '<td><a href="film.php?id=' . $value->id . '">' . $value->title . '</td>';
			$output .= '<td><a href="editmovie.php?id=' . $value->id . '">Redigera</td>';
			$output .=	"</tr>";
		}
		$output .= "</table>";
	...
	private function putPageLinks($pageURL, $queryPart){
		...
		if($this->currentPage > 1){	
			$str .= '<a href="' . $pageURL . $this->addKeyValsToOutURL("pag") . '&page=' . ($this->currentPage - 1);
			$str .= '"><</a>&nbsp;';
		}
				
		// page numbers
		for($i = 0; $i < ceil($this->resultRows / $this->handledReqVar["limit"]) ; $i++){
			$str .= "<a href='" . $pageURL . $this->addKeyValsToOutURL("pag");
			$str .= '&page=' . ($i+1); 
			$str .= " '>" . ($i+1) . "</a>&nbsp;"; 
		}
	...
	private function tableHeader(){
		$output = "<a href='" . $this->thisPage . $this->addKeyValsToOutURL("sortOrder, ascendOrDescend") . "&order=title" . "&ascDesc=a" . "'><img src='../../stelixx/webroot/img/asc.jpg'/></a>";
		$output .= "<a href='" . $this->thisPage . $this->addKeyValsToOutURL("sortOrder, ascendOrDescend") . "&order=title" . "&ascDesc=de" . "'><img src='../../stelixx/webroot/img/desc.jpg'/></a>";
		$output .= "</th>";
		$output .= "<th scope='col'>År";
		$output .= "<a href='" . $this->thisPage . $this->addKeyValsToOutURL("sortOrder, ascendOrDescend") . "&order=YEAR" . "&ascDesc=a" . "'><img src='../../stelixx/webroot/img/asc.jpg' /></a>";
		$output .= "<a href='" . $this->thisPage . $this->addKeyValsToOutURL("sortOrder, ascendOrDescend") . "&order=YEAR" . "&ascDesc=de" . "'><img src='../../stelixx/webroot/img/desc.jpg' /></a>";
		$output .= "</th>";
		return $output;	
	}
	...
	private function addKeyValsToOutURL($exceptVal){
		$output = "?";
		if(isset($this->handledReqVar["pag"]) AND strlen($this->handledReqVar["pag"]) > 0 AND strpos($exceptVal, "pag") < 0){
			$output .= '&page=' . $this->handledReqVar["pag"];	
		}
		if(isset($this->handledReqVar["limit"]) AND strlen($this->handledReqVar["limit"]) > 0 AND strpos($exceptVal, "limit") < 0){
			$output .= '&hits=' . $this->handledReqVar["limit"];	
		}
		if(isset($this->handledReqVar["txt"]) AND strlen($this->handledReqVar["txt"]) > 0 AND strpos($exceptVal, "txtSearch") < 0 ){
			$output .= '&txtSearch=' . $this->handledReqVar["txt"];
		}
		if(isset($this->handledReqVar["yearF"]) AND strlen($this->handledReqVar["yearF"]) > 0 AND strpos($exceptVal, "yearF") < 0){
			$output .= '&txtYearFrom=' . $this->handledReqVar["yearF"];	
		}
		if(isset($this->handledReqVar["yearT"]) AND strlen($this->handledReqVar["yearT"]) > 0 AND strpos($exceptVal, "yearT") < 0){
			$output .= '&txtYearTo=' . $this->handledReqVar["yearT"];	
		}
		if(isset($this->handledReqVar["sortOrder"]) AND strlen($this->handledReqVar["sortOrder"]) > 0 AND strpos($exceptVal, "sortOrder") < 0){
		...
		if(isset($this->handledReqVar["ascendOrDescend"]) AND strlen($this->handledReqVar["ascendOrDescend"]) > 0 
				AND strpos($exceptVal, "ascendOrDescend") < 0){
