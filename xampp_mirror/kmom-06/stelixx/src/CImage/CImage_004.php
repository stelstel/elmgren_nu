...
class CImage {
	...
	private $type, $attr, $image, $cacheFileName;
...
EOD;
	...
	private function calcNewDims(){	
		...
		if($this->cropToFit && $this->newWidth && $this->newHeight) {
			...
			if($this->verbose) { verbose("Crop to fit into box of {$newWidth}x{$newHeight}. Cropping dimensions: {$cropWidth}x{$cropHeight}."); }
		}
		else if($this->newWidth && !$this->newHeight) {
			$this->newHeight = round($this->newWidth / $aspectRatio);
			if($this->verbose) { verbose("New width is known {$newWidth}, height is calculated to {$newHeight}."); }
		}
		else if(!$this->newWidth && $this->newHeight) {
			$this->newWidth = round($this->newHeight * $aspectRatio);
			if($this->verbose) { verbose("New height is known {$newHeight}, width is calculated to {$newWidth}."); }
		}
		else if($this->newWidth && $this->newHeight) {
	...
	private function createFileName(){	
		...
		echo "<br>this->src: " . $this->src; ///////////////////////////////////////////////////////
		
		echo "<br>dirName: " . $dirName; ///////////////////////////////////////////////////////
		
		echo "<br>THIS_CACHE_PATH 001: " . CACHE_PATH; ///////////////////////////////////////////////////////
		
		echo "<br>THIS_CACHEFILENAME 001: " . $this->cacheFileName; ///////////////////////////////////////////////////////
		
		$this->cacheFileName = preg_replace('/^a-zA-Z0-9\.-_/', '', $this->cacheFileName);
		
		echo "<br>THIS_CACHEFILENAME 002: " . $this->cacheFileName; ///////////////////////////////////////////////////////
		...
		if(!$this->ignoreCache && is_file($this->cacheFileName) && $imageModifiedTime < $cacheModifiedTime) {
			...
			outputImage($this->cacheFileName, $this->verbose);
		}
