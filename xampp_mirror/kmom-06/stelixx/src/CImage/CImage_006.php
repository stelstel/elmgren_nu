...
class CImage {
	...
	private $type, $attr, $image, $cacheFileName, $maxWidth, $maxHeight, $targetRatio, $aspectRatio;
	...
	private function validateInput(){ // Validate incoming arguments
	...
	function outputImage($file, $verbose) {
		...
		if(isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == $lastModified){
			...
			readfile($file); //// STEL 001 //////////////////////////////////////////////////////////////////////////////
			//readfile("/home/saxon/students/20142/stel14/www/kmom-06/stelixx/webroot/img/cimage/cache/kodim15_350_233.png");//////////////////////
...
EOD;
	...
	private function calcNewDims(){	// Calculate new width and height for the image
		$this->aspectRatio = $this->width / $this->height;
		
		if($this->cropToFit && $this->newWidth && $this->newHeight) {
			$this->targetRatio = $this->newWidth / $this->newHeight;
			$cropWidth   = $this->targetRatio > $this->aspectRatio ? $this->width : round($this->height * $this->targetRatio);
			$cropHeight  = $this->targetRatio > $this->aspectRatio ? round($this->width  / $this->targetRatio) : $this->height;
	...
	private function createFileName(){	
		...
		//$this->cacheFileName = CACHE_PATH . "-{$dirName}-{$parts['filename']}_{$this->newWidth}_{$this->newHeight}{$this->quality_}{$this->cropToFit_}{$this->sharpen_}.{$this->saveAs}"; // <----- HÄR ÄR FELET
		$this->cacheFileName = CACHE_PATH . "{$parts['filename']}_{$this->newWidth}_{$this->newHeight}{$this->quality_}{$this->cropToFit_}{$this->sharpen_}.{$this->saveAs}";
