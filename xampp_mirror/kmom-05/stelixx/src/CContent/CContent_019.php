...
class CContent {
  ...
  public function __construct($db) {
  	$this->dBase = $db;
		...
		// $this->createContentTable($this->tableName); //################# Uncomment to create and populate table ######################
	}
...
EOD;
	...
	public function addPostOrPage($req){
		if(strlen($req) > 0){
			...
			if(strtolower($txtType) == "page"){
				...
				$type = "post";
				/*$sqlHighestBlogPost = "SELECT SUBSTRING(slug FROM 10) AS postIndex FROM " . $this->tableName . " WHERE TYPE = 'post' ORDER BY CONVERT(postIndex, UNSIGNED INTEGER) DESC LIMIT 0, 1";				
	...
	public function showUpdateForm($servQuery){
		...
		$sql = "SELECT * FROM " . $this->tableName . " WHERE id='" . $ide . "'";
		//echo "<br>sql:" . $sql . "<br>"; ////////////////////////////////////////////// 
		$result = $this->dBase->ExecuteSelectQueryAndFetchAll($sql);
		//echo "<br>result:" . dump($result) . "<br>"; ////////////////////////////////////////////// 
...
EOD;
  	...
  	$output .= '<input type="reset" name="btnReset" id="button" value="Återställ" />';
		$output .= '<p> <a href="aaa">Visa</a> | <a href="../webroot/kmom_05.php">Visa alla</a></p>';
		$output .= "</form>";
	...
	private function updateDB($servQuery){
		...
		if(isset($txtFilter) AND $this->isFilterValid($txtFilter)){
		...
		$sql .= "DATA='" . $text . "', ";
		$sql .= "FILTER='" . $filter . "', ";
		$sql .= "published='" . $publishedDate . "', ";
		$sql .= "updated='" . $updatedDate . "' WHERE id='" . $ide . "'";
		$queryResult = $this->dBase->ExecuteQuery($sql);
		
		if(!$queryResult){
			print_r($dBase->ErrorInfo());
		}
		return $queryResult;
	}
	...
	public function trash($query){
		...
		if(isset($delete) AND $delete == "true"){
			$sql = "DELETE FROM " . $this->tableName . " WHERE id=" . $id . " LIMIT 1"; //Limit 1 just for safety
			...
			$sql = "UPDATE " . $this->tableName . " SET deleted=NOW() WHERE id=" . $id;
