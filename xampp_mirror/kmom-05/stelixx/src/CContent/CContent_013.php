	...
	public function addPostOrPage($req){
		if(strlen($req) > 0){
			...
			if(strtolower($txtType) == "page"){
				...
				$sqlHighestBlogPost = "SELECT * FROM " . $this->tableName . " WHERE TYPE = 'post' ORDER BY slug DESC LIMIT 0, 1";
				$temp = $this->dBase->ExecuteSelectQueryAndFetchAll($sqlHighestBlogPost); 
				$highestBlogPost = substr($temp[0]->slug, strlen($blogPre) ); //Get highest blogpost number. ie blogpost-9
			...
			if(isset($txtText)){
				$text= htmlentities($txtText, null, 'UTF-8'); //Sanitizing 
			}else{
