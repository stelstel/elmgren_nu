...
EOD;
		...
		foreach($result AS $key=>$val){
			...
			$output .= '">visa</a>';
			$output .= ' <a href="' . $this->trashDelPage . '?trash=true&id=' . $id . '">släng</a>'; // %26=&
			$output .= ' <a href="' . $this->trashDelPage . '?delete=true&id=' . $id . '">radera</a>)</li>'; // %26=&
		}
		$output .= "</ul>";
...
EOD;
	...
	public function addPostOrPage($req){
		if(strlen($req) > 0){
			...
			$id = $title = $text = $slug = $url = $publDate = $createDate = $type = null;
			$filter = "nl2br"; // Default filter
			$blogPre = "blogpost-";
			...
			if(isset($txtFilter) AND strlen($txtFilter) > 0) {
