...
class CKmom04 {
  ...
  * Constructor
  */
  public function __construct($db, $reqURI, $dbTab) {
		echo "requri: " . $reqURI . "<br>"; ////////////////////////////////// 
		//$this->thisPage = null;
		
		$temp = explode("?", $reqURI);  // Split reqURI into page adress and request
...
EOD;
	...
	//***********************************************************************************
	private function makeSQLQuery($req){
		$txt = $yearF = $yearT = $limit = $pag = $sortOrder = $AscendOrDescend = null;
		$defaultLimit = 4;
		...
		if(isset($arrInputReq[txtSearch])){
			$txt = $arrInputReq[txtSearch];
		}
		
		if(isset($arrInputReq[txtYearFrom])){
			$yearF = $arrInputReq[txtYearFrom];
		}
		
		if(isset($arrInputReq[txtYearTo])){
			$yearT = $arrInputReq[txtYearTo];
		}
		...
		if(isset($arrInputReq[hits])){
			$limit = $arrInputReq[hits];
		}else{
			$limit = $defaultLimit;
		}
		
		if(isset($arrInputReq[page])){
			$pag = $arrInputReq[page];
		}else{
			$pag = 0;
		}
		
		if(isset($arrInputReq[order])){
			$sortOrder = $arrInputReq[order];
		}
		
		if(isset($arrInputReq[ascDes])){
			$AscendOrDescend = $arrInputReq[ascDesc];
		}
		
		
		//order=title&ascDesc=a		
		//[txtSearch] => %a%
//    [txtYearFrom] => 1900
//    [txtYearTo] => 2050
//    [hits] => 4
