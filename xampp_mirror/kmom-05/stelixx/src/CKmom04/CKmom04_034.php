...
EOD;
	...
	public function createResultTable() {
		...
		$this->resultRows = count($this->dataBase->ExecuteSelectQueryAndFetchAll("SELECT * FROM " . $this->dbTable ) );
		
		$result = $this->dataBase->ExecuteSelectQueryAndFetchAll($this->sqlQuery); 
		
		//echo "dump" . dump($result) . "<br>"; ////////////////////////////////////////////////////////
//		$this->resultRows = count($result);
	...
	private function putPageLinks($qry){
		...
		echo "thisresultRows" . $this->resultRows . "<br>"; //////////////////////////////////////////////////////////////
		
		if(isset($arrInputQry["limit"]) ){
			for($i = 0; $i < ceil($this->resultRows / $arrInputQry["limit"]) ; $i++){
